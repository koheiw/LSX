...
By default, `bootstrap_lss()` returns a sorted list of synonyms for each seed word. In each list, there is a seed word at the top followed by their synonyms identified in the corpus. Although there are many proper names in the example, we can find that many words are positive for positive seed words and negative for negative seed words. If a list is a mixture of positive and negative words, the seed word is probably too ambiguous.
...
If `mode = "coef"`, the function returns the similarity scores of words for each seed word. We can use the matrix to evaluate the seed words more systematically if lists of synonyms are not useful or sufficient.
