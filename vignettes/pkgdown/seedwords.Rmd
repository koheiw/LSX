...
By default, `bootstrap_lss()` returns a sorted list of synonyms for each seed word. In each list, there is a seed word at the top followed by its synonyms identified in the corpus. Although there are many proper names in the example, we can find many words are positive for positive seed words and negative for negative seed words. If a list is a mixture of positive and negative words, the seed word is probably too ambiguous.
